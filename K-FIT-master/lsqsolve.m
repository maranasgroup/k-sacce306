function [ xopt,fopt,fail,actcon,ropt,drdxopt,iter,restart ] = lsqsolve( x,model,A,b,actcon,debug,DFtoggle )
...
if nargin < 9
    [r,W,drdx,~,~,restart] = rescalc(x,model,debug); % initial function evaluation
end
...
while ~done && f0 > 0 
    ...
    if first
        ...
        if ~stepfail && isempty(blkcon) && (TermX || TermG)
            ...
            if TermX && debug
                disp('TermX');
                % kctherm only less than 5% for the perc_pass
                perc_pass = length(find(abs(g0.*x0) > 10))/length(x0);
                save('mylog.mat','perc_pass');
                fprintf('\n percent passed - %g\n',perc_pass);
            end
            if TermG && debug
                disp('TermG');
    ...
    if h > hmin && ~stepfail
        x = x0 + h*dx;
        [r,~,drdx,~,~,restart] = rescalc(x,model,debug);
        f = r'*W*r;
        iter = iter+1;
        r1 = r0 + drdx0*dx*h;
        f1 = r1'*W*r1;
...
%outputs
model.fstore = f0; %MH edit 06152021
