function [ ybest,complete,jfail,tf] = SIFOInteg( y0,model,ipert,sthresh,debug )
...
if q <sthresh
    if debug 
        fprintf('SIFOInteg - q within threshold \n');
    end
    done = true;
    complete = true;
else
...
while ~done
    ...
    if q<sthresh
        if debug
            fprintf('SIFOInteg criteria 1 - ctx %g\n',ctx);
        end
        done = true;
        complete = true;
        jfail = false;
    ...
    elseif ctx>200
        if debug
            fprintf('SIFOInteg max iteration reached\n');
        end
        done = true;
        jfail = false;
        complete = false;
