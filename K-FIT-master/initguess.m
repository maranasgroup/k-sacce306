function [ x  ] = initguess( model,A,b,xlb,xub,counter )
...
if model.options.reinit
    fprintf('rng-seed %f, iter - %g\n',rngseed.Seed,mod(counter,50));
end
for i=0:mod(counter,50)
     x = rand(nvar,1);
end
% changed from 100 to 1000 to cover entire flux distribution
x(~inds) = 1000*x(~inds);
save('inds_flux.mat');
