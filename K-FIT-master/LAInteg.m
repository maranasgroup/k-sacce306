function [ yf,complete,restart ] = LAInteg( y0,model,ipert,sthresh,first,cind,debug )
...
while ~done
    ...
    if first && err < 1 && h > 1e12 && h1/hx < 1.2
        ...
        complete = false;
        if debug
            fprintf('LAInteg criteria 1 , iterations %g\n',ctx);
        end
    ...
    elseif err<1 && h>1e15 && h1/hx < 1.2
        ...
        complete = false;
        if debug
            fprintf('LAInteg criteria 3 , iterations %g\n',ctx);
        end
    elseif ctx > 5000
        %restart = true;
        fprintf('LAInteg - max iteration (5000) exceeded\n');
        error('LAInteg - max iteration (5000) exceeded');
        break
    end
