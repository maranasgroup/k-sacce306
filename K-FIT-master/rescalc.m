function [r,W,J,vop,cs,restart] = rescalc(x,model,debug)
format shortG
global fpi integration newton
...
for i = 1:ncond
    ...
    while ~done && ~restart
        ...
        %css = c0;
        iter = 0;
        while ~complete && ~restart
            iter = iter+1;
            fpistart = tic;
            [cs(:,i),~,complete] = svsucsubs(c0,model,i,sthresh,debug);
            fpi = fpi + toc(fpistart);
            if ~complete
                ...
                %end
                newtonstart = tic;
                while ~proceed && ~restart
                    [cs(:,i),complete,restart] = LAInteg(cs(:,i),model,i,sthresh,true,cind,debug);
                    jfail = false;
                    if ~complete
                        [cs(:,i),complete,jfail] = SIFOInteg(cs(:,i),model,i,sthresh,debug);
                        if jfail && ~restart
                            iter = iter-1;
                            [cs(:,i),complete,restart] = LAInteg(cs(:,i),model,i,sthresh,false,cind,debug);
                        end
                    end
                    if (complete || ~jfail) && ~restart
                        proceed = true;
                    end
                end
                newton = newton + toc(newtonstart);
            %else
            %    css = ci;
            end
            if iter>3
        ...
        integration = integration + toc(istart);
