function [ css,vss,complete ] = svsucsubs( c0,model,ipert,sthresh ,debug)
...
while ~done
    ...
    %if 10000*abs(q-q0)<1
        if iter > 1000
            fprintf('svsucsubs - max iteration reached, ctr -%g\n',ctr);
        end
        ctr = ctr+1;
    else
        ctr = 0;
    ...
    %if max(abs(ck1-ck))<1e-10 || max(abs(dc))<1e-8
        fprinf('svsucsubs - ss reached at iteration %g\n',iter);
        done = true;
        complete = true;
        css = ck1;
        vss = v;
